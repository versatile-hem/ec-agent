...
public class LoginView extends VerticalLayout {
     ...
     * stars.getChildren().forEach(c -> ((Icon)
     * c).setColor("var(--lumo-warning-color)"));
     * card.add(stars);
     * 
     ...
     * boolean valid = email.getValue() != null &&
     * email.getValue().matches("^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$")
     * && password.getValue() != null && !password.getValue().isBlank();
     * 
     * if (!valid) {
     * Notification.show("Please provide a valid email and password.", 3000,
     ...
     * } else {
     * Notification.show("Invalid credentials.", 3000,
     * Notification.Position.TOP_CENTER);
     * }
     * }
     */
